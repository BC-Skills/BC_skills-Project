...
export default function Formations() {
    ...
    const [selectedFormationTypeid, setSelectedFormationTypeid] =
        useState(null);
    ...
    useEffect(() => {
        ...
        const hasFormationLink = parsedLinks.some(
            (link) => link.name === "formation"
        );
        fecthformtiontype();
        if (!hasFormationLink) {
            navigate("/users");
    ...
    const handleDelete = (formationTypeId) => {
        if (
            window.confirm(
                "Are you sure you want to delete this formation type?"
            )
        ) {
            axiosClient
                .delete(`formation-types/${formationTypeId}`)
                .then(() => {
                    // Update the list of formation types after successful deletion
                    setFormationTypes((prevFormationTypes) =>
                        prevFormationTypes.filter(
                            (type) => type.id !== formationTypeId
                        )
                    );
                })
                .catch((error) => {
                        ...
                        <div className="p-8 sm:p-9 md:p-7 xl:p-9 text-center">
                            ...
                            <p className="text-base text-body-color leading-relaxed mb-7 ">
                                {showMore[formationType.id]
                                    ...
                                    : formationType.description.substring(
                                          0,
                                          100
                                      ) + "..."}
                                <button
                                    onClick={() =>
                                        toggleShowMore(formationType.id)
                                    }
            ...
            <div className="flex justify-center mt-4">
                <ul className="flex">
                    {Array.from(
                        {
                            length: Math.ceil(
                                formationTypes.length / formationPerPage
                            ),
                        },
                        (_, index) => (
                            <li key={index}>
