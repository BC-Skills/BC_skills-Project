...
const CourseModal = ({ isOpen, formationType, onClose }) => {
  ...
  const handleDownload = async (formationId, fileName) => {
    try {
        const response = await axiosClient.get(`formations/${formationId}/download`, {
  ...
  return (
    <div className={`course-modal ${isOpen ? "visible" : "hidden"}`}>
      <div className="flex modal-container w-[1000px] flex-col pb-2 bg-white h-[900px] md:max-w-md mx-auto rounded shadow-lg z-50 overflow-y-auto">
  ...
  <div className="table">
    {formationType.formations.map((formation, index) => (
